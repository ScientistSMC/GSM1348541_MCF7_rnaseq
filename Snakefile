...
rule all:
	input:
		expand(FASTQC_TRIM_DIR+"{sample}_trimmed_fastqc.zip", sample=config["samples"]),
        #expand(FASTQC_TRIM_DIR+"{sample}_R2_001_val_2_fastqc.zip", sample=config["samples"]),
        expand(FASTQC_DIR+"{sample}_fastqc.html", sample=config["samples"]),
        #expand(FASTQC_DIR+"{sample}_R2_001_fastqc.html", sample=config["samples"]),
        expand(FASTQ_TRIM+"{sample}_trimmed.fq", sample=config["samples"]),
        #expand(FASTQ_TRIM+"{sample}_R2_001_val_2.fq.gz", sample=config["samples"]),
...
rule fastqc:
    input:
        r1=FASTQ+"{sample}.fastq",
		#r2=FASTQ+"{sample}_R2_001.fastq.gz"
    output:
        html1=FASTQC_DIR+"{sample}_fastqc.html",
        zip1=FASTQC_DIR+"{sample}_fastqc.zip",
		#html2=FASTQC_DIR+"{sample}_R2_001_fastqc.html",
        #zip2=FASTQC_DIR+"{sample}_R2_001_fastqc.zip"
    conda:
...
rule multiqc_fastqc:
    input:
        fastqc=FASTQC_DIR,
        r1=expand(FASTQC_DIR+"{sample}_fastqc.html", sample=config["samples"]),
        #r2=expand(FASTQC_DIR+"{sample}_R2_001_fastqc.html", sample=config["samples"])
    output:
        "multiqc_fastqc/multiqc_report.html"
    conda:
        "envs/rna-seq_conda_env.yaml"
...
rule fastq_trim:
    input:
        ...
        #r2=FASTQ+"{sample}_R2_001.fastq.gz"
    output:
        r1=FASTQ_TRIM+"{sample}_trimmed.fq",
        #r2=FASTQ_TRIM+"{sample}_R2_001_val_2.fq.gz",
        r1_trim_report=FASTQ_TRIM+"{sample}.fastq_trimming_report.txt",
        #r2_trim_report=FASTQ_TRIM+"{sample}_R2_001.fastq.gz_trimming_report.txt"
    conda:
...
rule fastqc_trimmed:
    input:
        FASTQ_TRIM+"{sample}_trimmed.fq",
        #FASTQ_TRIM+"{sample}_R2_001_val_2.fq.gz"
    output:
        html=FASTQC_TRIM_DIR+"{sample}_trimmed_fastqc.html",
        zip=FASTQC_TRIM_DIR+"{sample}_trimmed_fastqc.zip",
        #html2=FASTQC_TRIM_DIR+"{sample}_R2_001_val_2_fastqc.html",
        #zip2=FASTQC_TRIM_DIR+"{sample}_R2_001_val_2_fastqc.zip"
    conda:
        "envs/rna-seq_conda_env.yaml"
    shell:
...
rule multiqc_fastqc_trimmed:
    input:
        ...
        html=expand(FASTQC_TRIM_DIR+"{sample}_trimmed_fastqc.html", sample=config["samples"]),
        zip=expand(FASTQC_TRIM_DIR+"{sample}_trimmed_fastqc.zip", sample=config["samples"])
    output:
        "multiqc_fastqc_trimmed/multiqc_report.html"
    conda:
        "envs/rna-seq_conda_env.yaml"
